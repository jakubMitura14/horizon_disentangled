...
class Mock3DDataset(Dataset):
    ...
    def __getitem__(self, idx):
        # Increased 3D Volume: [C, D, H, W] -> 96x96x64
...
class HeavyResNet3D(pl.LightningModule):
    def __init__(self):
        super().__init__()
        self.in_channels = 64
    ...
    def _make_layer(self, out_channels, blocks, stride):
        layers = []
        layers.append(ResNetBlock3D(self.in_channels, out_channels, stride))
    ...
    def training_step(self, batch, batch_idx):
        output = self(batch)
        loss = output.mean() # Dummy scalar loss
...
def main():
    ...
    dataloader = DataLoader(dataset, batch_size=4) # Larger batch size
