...
\subsubsection*{A.2 Cloud Computing Costs}
The project will employ a hybrid computing model that combines secure, on-premise resources for data handling with a flexible, scalable cloud environment for high-performance computing, representing the central scientific instrument for this research. This strategic approach ensures maximum data security while providing the adaptability needed for computationally intensive research. The budget of \textbf{\EUR{321,600}} is allocated for on-demand access to a leading cloud provider (e.g., AWS, Google Cloud, or Azure) for model training and experimentation.

\paragraph{1. Strategic Imperative and Justification for Hybrid Computing Model}
A hybrid on-premise and cloud strategy is strategically superior for a project involving sensitive medical data and cutting-edge AI research. This model provides the best of both worlds:
\begin{itemize}
    \item \textbf{On-Premise for Security (DIZ):} For critical reasons of data security and patient privacy, the main storage of raw clinical data and the initial preprocessing and anonymization steps will be handled locally within the secure environment of the applicant's Data Integration Center (DIZ). This process will leverage the robust, validated tools and infrastructure developed as part of the SIKIT project, ensuring compliance with institutional and GDPR requirements from the outset.
    \item \textbf{Cloud for Scalability and Flexibility (HPC):} Once data is fully anonymized, it will be securely transferred to a cloud environment for the computationally demanding tasks of model training, hyperparameter optimization, and large-scale validation experiments. The project's computational needs will vary significantly across its lifecycle:
    \begin{itemize}
        \item \textbf{Mid Stages (WP3-WP4):} Will see a surge in demand for high-VRAM, multi-GPU instances to train the complex VAE and Neural Jump ODE models on large-scale 3D imaging data.
        \item \textbf{Late Stages (WP5):} Will require a mix of GPU resources for model refinement and CPU-intensive instances for running validation studies and deploying user-facing demonstration tools.
    \end{itemize}
\end{itemize}
This hybrid approach allows us to dynamically provision the optimal hardware for each specific research task in the cloud—paying only for what we use—while guaranteeing that sensitive patient data remains protected within our secure institutional infrastructure. This adaptability is critical for de-risking the project's ambitious technical goals and ensuring the most efficient and secure use of funds.

\textbf{Total Estimated Cloud Computing Costs: \EUR{321,600}}
