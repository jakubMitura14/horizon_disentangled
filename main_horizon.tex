...
\subsection{Outcomes}
This project is designed to generate specific, measurable, and credible impacts for clearly defined target groups. Our pathways to outcomes are grounded in the project's concrete deliverables and are designed to be traceable and significant within the project's timeframe.
...
\toprule
\textbf{Risk Analysis and Mitigation Strategies} \\
\midrule
\endfirsthead
\endhead

\textbf{Risk 1 (Operational): Training Instability and Data Scalability} \\
...
\textbf{Risk 2 (Operational): Generalizability and Overfitting} \\
...
\textbf{Risk 3 (Operational): Performance with Incomplete and Heterogeneous Data} \\
\textit{Likelihood: High, Impact: High} \\
...
\textbf{Risk 4 (Conceptual): High Project Complexity} \\
\textit{Likelihood: Medium, Impact: High} \\
\textbf{Risk:} The project's ambition to create a comprehensive causal framework entails significant scientific and technical complexity, which could lead to delays or failure to achieve all objectives. \\
\textbf{Mitigation:} The project's complexity is managed by deconstructing it into a modular, four-stage framework, with each stage corresponding to a well-defined Work Package. Each component is based on methods with established, published implementations, which reduces the risk of technical failure and allows for parallel development. \\
\textbf{Expected Effect:} This structured approach makes the project tractable, allowing for clear progress monitoring and reducing the risk of catastrophic failure by containing potential issues within specific modules. \\
\addlinespace

\textbf{Risk 5 (Operational): Data Scarcity or Lack of Diversity} \\
\textit{Likelihood: Low, Impact: High} \\
\textbf{Risk:} Insufficient access to large, diverse, and well-annotated datasets could limit the model's training and validation, compromising its performance and generalizability. \\
\textbf{Mitigation:} This risk is low, as the applicant has direct access to one of Europe's largest single-center datasets for theranostics. Furthermore, the project is built on established collaborations with multiple university clinics (Halle, Charité) and radiological practices, ensuring a large and diverse data pool from the outset. \\
\textbf{Expected Effect:} Access to this rich, multi-center data ensures the model can be trained on a representative sample of the patient population, significantly enhancing its robustness and clinical relevance. \\
\addlinespace

\textbf{Risk 6 (Conceptual): Failure due to Low Starting TRL} \\
\textit{Likelihood: High, Impact: High} \\
\textbf{Risk:} The project's core causal framework begins at TRL 1, which signifies fundamental research with a high inherent risk of not achieving the envisioned breakthrough. \\
\textbf{Mitigation:} We mitigate this by building upon a TRL 3 foundation for data processing and management derived from the SIKIT project (€1.29M funding). This allows us to de-risk the initial project phases and focus our resources on the novel, high-risk TRL 1 research into the causal framework itself. \\
\textbf{Expected Effect:} This strategic leveraging of existing projects provides a solid technical bedrock, increasing the probability of success for the most ambitious and innovative parts of the project. \\
\addlinespace

\textbf{Risk 7 (Operational): Failure of Clinical Adoption} \\
\textit{Likelihood: Medium, Impact: High} \\
\textbf{Risk:} Even if technically successful, the framework may not be adopted by clinicians if it is too slow for real-time use or if it faces insurmountable regulatory hurdles. \\
\textbf{Mitigation:} We address this proactively. 1) **Computational Feasibility:** Our latent-space modeling approach (WP3, WP4) is designed to be computationally efficient, making the tool fast enough for real-time clinical use. 2) **Regulatory Hurdles:** We employ a "compliance-by-design" approach, aligning with the EU AI Act from the project's outset (WP7) to streamline future certification. \\
\textbf{Expected Effect:} This dual strategy increases the likelihood of successful translation from a research prototype to a clinically adopted tool by ensuring it is both practical for the workflow and prepared for the regulatory environment. \\
\addlinespace

\textbf{Risk 8 (Operational): Model Drift and Performance Deterioration} \\
\textit{Likelihood: Medium, Impact: High} \\
\textbf{Risk:} The model's performance could degrade over time as clinical practices, imaging technologies, or patient populations change, a phenomenon known as model drift. \\
\textbf{Mitigation:} Grounding the model in fundamental biological knowledge through disentanglement (WP3) makes it inherently more robust to superficial data shifts. Furthermore, WP5 includes the development of a continuous monitoring and validation plan to detect performance degradation early and trigger model retraining as needed. \\
\textbf{Expected Effect:} This ensures the long-term reliability and safety of the model, maintaining its accuracy and trustworthiness as the clinical environment evolves. \\
\bottomrule
\end{longtable}
...
\end{itemize}
The project will be led by Prof. Dr. Michael Kreißl, a renowned expert in nuclear medicine, who brings unparalleled leadership experience as the head of the working group for the S3-guidelines on thyroid cancer in Germany. This direct experience in shaping national clinical standards is a unique asset that will guide the project's clinical and regulatory strategy. He will be supported by a co-coordinator with a dual specialization in nuclear medicine and radiology, ensuring that a deep, multi-modal clinical perspective is embedded in the project's core. A medical doctor with a specialization in nuclear medicine and an IT background will provide additional coordination. The applicant's team has extensive, demonstrated experience in the core technologies required for this project, including synthetic CT creation from nuclear imaging, clinical decision support systems, and semi-automatic image segmentation pipelines. This deep clinical and technical expertise, combined with our strong collaborative network, uniquely positions us to succeed in this ambitious project.
