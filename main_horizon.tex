...
\begin{itemize}
    ...
    \begin{itemize}
        ...
        \item \textbf{At the Architectural Level:} The core of our knowledge integration lies in the principled disentanglement of the \gls{vae}'s latent space. This approach mirrors the clinical reasoning process where physicians first analyze the \textbf{disease state} (e.g., TNM stage, tumor volume) and the overall \textbf{patient state} (e.g., age, frailty, comorbidities) as separate components, often using different specialized tools and diagnostic scales for each. Only after this partial, separate analysis do they mentally integrate these streams of information to form a holistic assessment. Our model operationalizes this exact workflow by architecturally separating these factors into distinct latent subspaces before integrating them for a final prediction. Furthermore, this technique is used to explicitly disentangle and isolate known technical \textbf{confounders}, such as differences in scanner type or imaging protocol, ensuring the learned disease representation is robust to these variations.
...
\subsubsection{VAE-based Out-of-Distribution Detection for Label Noise Resilience}
...
\begin{table}[H]
    ...
    \begin{tabular}{p{0.25\textwidth} p{0.35\textwidth} p{0.35\textwidth}}
        ...
        \midrule
        \textbf{Stage 0: Data Quality Assurance} & The VAE-based OOD detection model will be evaluated on its ability to distinguish between a clean validation set and a validation set seeded with synthetic anomalies. Metrics will include AUC for OOD detection and precision/recall for identifying the anomalous samples. & The flagged outliers will be reviewed by clinical experts. The percentage of clinically-confirmed outliers will serve as a measure of the system's real-world effectiveness. The impact of this cleaning step will be measured by the performance improvement of downstream models. \\
        \addlinespace
...
\begin{itemize}
    ...
    \item \textbf{Outcome 2: Increased Efficiency and Reduced Administrative Burden for Clinical Staff.}
        \begin{itemize}
            ...
            \item \textbf{Benefit:} The system will auto-populate reports with prognostic predictions, treatment simulations, and key imaging findings. We will measure the time saved in the clinical workflow study (WP5), with an expected reduction of 10-15 minutes of manual documentation per patient case, freeing up clinician time for direct patient care. The counterfactual visualizations will also allow radiologists to more quickly understand the AI's reasoning, with a target of reducing the time to interpret complex cases by 25\%.
        \end{itemize}
...
\begin{table}[H]
    ...
    \begin{tabular}{lcccccccc|c}
        ...
        \midrule
        \textbf{Person-Months} & 30 & 50 & 60 & 60 & 30 & 10 & 10 & 10 & \textbf{282} \\
        \bottomrule
    \end{tabular}
\end{table}
