...
The project will leverage a flexible, scalable cloud computing model for all high-performance computing needs, representing the central scientific instrument for this research. This approach avoids a large, upfront capital investment in on-premise hardware and instead provides a dynamic and adaptable infrastructure that can evolve with the project's changing requirements. The budget of \textbf{\EUR{321,600}} is allocated for on-demand access to a leading cloud provider (e.g., AWS, Google Cloud, or Azure).

\paragraph{1. Strategic Imperative and Justification for Cloud Computing}
A cloud-based strategy is strategically superior for a high-risk, high-gain research project like this one. The primary justification is \textbf{flexibility}. The project's computational needs will vary significantly across its lifecycle:
\begin{itemize}
    \item \textbf{Early Stages (WP1-WP2):} Will require significant storage and data preprocessing capabilities, with moderate GPU usage for initial supervisor model training.
...
\end{itemize}
A cloud provider allows us to dynamically provision the optimal hardware for each specific task—from large-memory instances for data loading to the latest GPU architectures for deep learning—paying only for what we use. This eliminates the risk of being locked into a single on-premise hardware configuration that may become suboptimal as the research evolves or new technologies become available. This adaptability is critical for de-risking the project's ambitious technical goals and ensuring the most efficient use of funds.
