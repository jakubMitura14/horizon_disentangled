...
The project's objectives are designed to be specific, measurable, achievable, relevant, and time-bound (SMART), and are explicitly aligned with the expected outcomes of the EIC Pathfinder Challenge.
\begin{enumerate}
    \item \textbf{To Pioneer a Causal, Longitudinal Model of Prostate Cancer (EIC Outcome 1):} The core scientific objective is to develop and validate a novel Causal AI framework, built upon a Neural Jump Ordinary Differential Equation (NJDE) architecture. This model will be capable of learning the underlying dynamics of disease progression from sparse, irregularly-sampled, multi-modal data, while explicitly modeling the impact of clinical interventions like surgery and therapy. This foundational technology will establish a new European standard for clinical AI.

    \item \textbf{To Achieve True Explainability through Counterfactual Reasoning (EIC Outcome 2):} We will move beyond opaque "black-box" models by building a system that can generate clinically plausible, visual counterfactuals. This will enable clinicians to ask "what-if" questions and receive intuitive, visual explanations for the model's predictions, fostering the clinical trust necessary for widespread adoption and establishing a new paradigm for trustworthy AI.

    \item \textbf{To Master Heterogeneity through Principled Disentanglement (EIC Outcome 1):} A key technical objective is to develop a robust Variational Autoencoder (VAE) framework that can disentangle the core components of medical imaging data. The model will learn to separate true disease signals from patient-specific anatomy, age-related changes, and technical confounders (e.g., scanner type, site-specific artifacts), ensuring the model is robust, generalizable, and ready for deployment across diverse European healthcare systems. The framework will be designed to be extensible, allowing for the incorporation of histopathological images, genetic information, and other omics data (e.g., radiomics, proteomics) when available.

    \item \textbf{To Create a Dynamic "Digital Twin" for Personalized Prognosis (EIC Outcome 3):} We aim to deliver a tool that can simulate a patient's future disease trajectory under various scenarios. The model will generate predictions not only of future scans but also of key clinical endpoints and structured radiological reports, providing a comprehensive, personalized prognostic tool that has the potential to revolutionize patient management.

    \item \textbf{To Quantify Uncertainty for Safer Clinical Decisions (EIC Outcome 2):} We will develop and integrate robust methods for uncertainty quantification directly into our framework. The model will not only provide predictions but also a reliable measure of its own confidence, a critical feature for safe clinical use. This allows the system to "know what it doesn't know," flagging high-uncertainty cases for closer human review and preventing over-reliance on automated outputs.
    
    \item \textbf{To Validate the Framework in a Real-World Clinical Context (EIC Outcome 3):} The project will culminate in a rigorous evaluation of the model's performance, using both quantitative metrics (e.g., predictive accuracy, counterfactual quality) and a qualitative, clinician-in-the-loop study to assess its real-world clinical plausibility, utility, and impact on diagnostic workflows, thereby paving the way for its integration into the European healthcare ecosystem.

    \item \textbf{To Develop a Treatment-Choice Helper (EIC Outcome 3):} A key objective is to create a clinical decision support tool that can select the optimal treatment option or combination, as well as the ideal timing for interventions. This will be achieved by designing interventions that are explicitly optimized to maximize the expected time to progression, providing clinicians with data-driven strategies for patient management.

\item \textbf{To Accelerate Clinical Research and Trial Design (EIC Outcome 4):} The model will serve as a powerful "digital twin," enabling in-silico experiments to generate and pre-test research hypotheses for designing new clinical controlled trials. By modeling virtual interventions, the framework can help resolve clinical uncertainties, such as the optimal use of Prostate-Specific Membrane Antigen (PSMA) Positron Emission Tomography (PET), saving time, money, and minimizing patient exposure to unnecessary risks.
\end{enumerate}
...
\begin{itemize}
    \item \textbf{A New European Standard for Trustworthy Clinical AI:} We will pioneer a shift away from correlational "black-box" models towards truly causal and explainable AI. The development of a robust, generalizable framework for learning causal models from observational clinical data will represent a landmark achievement. This provides a blueprint for future research in other complex diseases (e.g., Alzheimer's, diabetes) and establishes a new European standard for the development and deployment of trustworthy AI systems in high-stakes environments, strengthening Europe's technological leadership. The primary beneficiaries will be the European AI research community, medical technology companies, and regulatory bodies.
    \item \textbf{Advancing the Frontier of Generative AI:} Our work on principled disentanglement, counterfactual generation, and longitudinal modeling with Neural Jump ODEs will push the boundaries of generative AI. By creating a model that can reason about cause and effect, we are developing a foundational technology with applications far beyond medicine. The novel techniques developed will be of immense value to the broader European AI community, benefiting researchers in fields such as climate modeling and financial forecasting.
    \item \textbf{Fostering an Open and Sovereign European AI Ecosystem:} We are deeply committed to open science and Europe's digital sovereignty. We will make our code publicly available under a permissive open-source license (e.g., Apache 2.0) via a public GitHub repository. To ensure compliance with all data protection laws, we will contribute our curated, anonymized, and harmonized datasets to the \textbf{European Cancer Imaging (EUCAIM)} platform. This strategy ensures our work can foster further research and innovation across the EU while upholding the strictest data privacy standards.
    \item \textbf{Enhanced Diagnostic Accuracy and Personalised Treatment:} By providing clinicians with a "digital twin" that can simulate disease trajectories and treatment responses, our framework will enable more accurate staging, better risk stratification, and truly personalized treatment planning. This will help to reduce both over- and under-treatment—a critical issue in prostate cancer—thereby minimizing treatment-related side effects and significantly improving patient quality of life. The direct beneficiaries are prostate cancer patients across Europe, as well as the oncologists and radiologists responsible for their care.
    \item \textbf{Empowering Clinicians and Reducing Workload:} The model's ability to generate intuitive, counterfactual explanations and automated structured reports will empower clinicians, augmenting their decision-making process and freeing up valuable time from tedious documentation. This will improve workflow efficiency in urology and radiology departments, reduce burnout, and allow clinicians to focus on what matters most: patient care.
    \item \textbf{A Foundation for a New Era in European Oncology:} While focused on prostate cancer, the foundational technology developed in this project is modality-agnostic and disease-agnostic. It has the potential to be adapted to other cancers (e.g., breast, lung) and complex chronic diseases, paving the way for a new era of data-driven, causal medicine that is more personalized, more effective, and more explainable, with Europe at the forefront. This will benefit the entire European healthcare sector.
\end{itemize}
...
\begin{itemize}
    \item \textbf{High-Impact Scientific Publications:} We will target leading peer-reviewed journals such as Nature Medicine, The Lancet Digital Health, and the Journal of Clinical Oncology, and present our findings at premier international conferences including the Medical Image Computing and Computer Assisted Intervention (MICCAI) conference, the Radiological Society of North America (RSNA) Annual Meeting, and the European Association of Nuclear Medicine (EANM) Congress.
    \item \textbf{Open Source and Open Data:} As stated, all code will be open-sourced via a public GitHub repository under a permissive Apache 2.0 license. In line with our data governance strategy, we will contribute our curated, anonymized datasets to the \textbf{European Cancer Imaging (EUCAIM)} platform as a Tier 3 Federated Node. This commitment ensures our data legacy strengthens the European research community while respecting data protection regulations.
    \item \textbf{Engagement with the Clinical and Patient Community:} We will actively engage with European clinical societies such as the European Association of Urology and the European Society for Medical Oncology, as well as patient advocacy groups like Europa UOMO, to ensure our work is aligned with real-world clinical needs and to facilitate its translation into clinical practice across the EU.
    \item \textbf{Commercial Exploitation and Standardization:} We will explore pathways for commercial exploitation through partnerships with leading European medical imaging companies like Siemens Healthineers and Philips, and with innovative health-tech startups. Furthermore, we will actively participate in standardization bodies such as the European Institute for Biomedical Imaging Research (EIBIR) and the Digital Imaging and Communications in Medicine (DICOM) working groups to promote the adoption of our structured reporting and causal AI frameworks as a new European standard for trustworthy clinical AI.
\end{itemize}

\section{Implementation}
\subsection{Applicant’s Team Profile and Clinic Expertise}
The applicant institution is uniquely positioned to succeed in this high-risk, high-gain endeavor. The applicant, the Universitätsklinik für Radiologie und Nuklearmedizin, possesses not only the requisite multimodal data and a proven track record in developing advanced AI models, but also the critical clinical expertise to ensure the solution's reliability. To ensure clear leadership and accountability, one of the medical doctors with a dual specialization in both nuclear medicine and radiology will serve as the single Principal Investigator (PI), with the full authority to take scientific responsibility for the work, manage the allocated resources, and ensure project delivery with institutional backing. The second medical doctor, who combines a specialization in nuclear medicine with a Master's degree in IT, will act as a co-coordinator, with defined responsibilities for the technical development and implementation of the AI framework.

Our approach is grounded in the principle that diagnostic data (PET/MR, CT, Electronic Health Records (EHR) etc.) contains the key patient information, and our advantage as nuclear medicine specialists and radiologists is the deep understanding of its interpretation and central importance in the diagnostic chain. The project team is working on projects involving among others:
\begin{itemize}
...
\begin{figure}[H]
    \centering
    \includegraphics[trim=5mm 15mm 5mm 25mm, % {left bottom right top}
        clip,                     % This command applies the trim
...
\begin{itemize}
    \item \textbf{WP1: Data Curation and Harmonization (30 Person-Months):} This foundational work package focuses on gathering, anonymizing, and harmonizing the multi-modal data from all collaborating sites. It includes the development of a robust data pipeline and the implementation of the data governance framework compliant with GDPR and the AI Act.
    \item \textbf{WP2: Foundational Supervisor Model Training (50 Person-Months):} This WP is dedicated to developing the suite of "supervisor" models that will provide the clinical ground truth for the main generative model. This includes training ordinal classifiers, censored survival regressors, and models for anatomical segmentation and confounder identification.
    \item \textbf{WP3: Causal VAE Development (60 Person-Months):} This is a core research WP focused on developing the hierarchical Variational Autoencoder (VAE) for principled disentanglement. This includes the design and implementation of the VAE architecture and the targeted total correlation loss function.
    \item \textbf{WP4: Temporal Trajectory Modeling (60 Person-Months):} This WP will develop the Neural Jump Ordinary Differential Equation (NJDE) framework to model the longitudinal disease trajectories. It involves training the model to learn the continuous dynamics of the disease and the discrete "jumps" caused by clinical interventions.
    \item \textbf{WP5: Validation and Clinical Integration (30 Person-Months):} This WP focuses on the rigorous evaluation of the complete framework. It includes the quantitative validation of the model's predictive performance and the qualitative, clinician-in-the-loop studies to assess clinical plausibility and workflow integration.
    \item \textbf{WP6: Dissemination, Communication, and Exploitation (10 Person-Months):} This WP will manage all activities related to maximizing the project's impact, including scientific publications, conference presentations, open-source code releases, and engagement with clinical, patient, and industry stakeholders.
    \item \textbf{WP7: Project Management (10 Person-Months):} This WP covers the overall coordination of the project, including administrative and financial management, risk management, and ensuring compliance with all legal and ethical requirements.
    \item \textbf{WP8: Portfolio Activities (10 Person-Months):} This WP is dedicated to fostering collaboration and synergies across the EIC Pathfinder Challenge portfolio. Activities will include contributing to the portfolio's strategic plan, participating in joint workshops, benchmarking activities, and collaborative dissemination and exploitation efforts, as outlined in the Challenge Guide.
\end{itemize}
...
%     \begin{itemize}
%         \item Approximately 150-200 longitudinal studies of patients with paired baseline $^{\text{68}}\text{Ga-PSMA PET/CT}$ and mpMRI scans for primary staging and follow-up.
%         \item A unique cohort of approximately 150 patients with advanced disease who have undergone $^{\text{177}}\text{Lu-PSMA}$ radioligand therapy (RLT). For these patients, we have paired pre-therapy $^{\text{68}}\text{Ga-PSMA PET/CT}$ scans and post-therapy $^{\text{177}}\text{Lu-PSMA}$ SPECT/CT scans, which are essential for modeling treatment response.
%         \item A very unique cohort of interim therapy controls based on $^{\text{68}}\text{Ga-PSMA PET/CT}$ and CT –pairs from 40 patients.
%         \item An additional cohort of 200 patients with $^{\text{68}}\text{Ga-PSMA PET/CT}$ scans performed at two different time points.
%     \end{itemize}
...
%     \begin{itemize}
%         \item Approximately 100 additional paired $^{\text{68}}\text{Ga-PSMA PET/CT}$ and mpMRI studies.
%         \item Approximately 100 additional paired pre-therapy $^{\text{68}}\text{Ga-PSMA PET/CT}$ and post-therapy $^{\text{177}}\text{Lu-PSMA}$ SPECT/CT studies.
%     \end{itemize}
...
%     \begin{itemize}
%         \item Approximately 100 additional paired $^{\text{68}}\text{Ga-PSMA PET/CT}$ and mpMRI studies.
%         \item Approximately 300 additional paired pre-therapy $^{\text{68}}\text{Ga-PSMA PET/CT}$ and post-therapy Lutetium SPECT/CT studies.
%     \end{itemize}
...
\subsection{Resources and Budget Justification}
