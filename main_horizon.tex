...
\begin{figure}[H]
    \centering
    \includegraphics[        trim=30mm 30mm 30mm 30mm, % {left bottom right top}
        clip,                     % This command applies the trim
        width=\textwidth]{graphical_abstract.png}
    \caption{Objectives and concept}
...
\textbf{Mitigation:} The project's complexity is managed by deconstructing it into a modular, four-stage framework. Each component is based on methods with established, published implementations, which reduces technical risk. We further mitigate this by building upon a TRL 3 foundation for data processing derived from the SIKIT project (€1.29M funding), de-risking the initial phases. \\
...
Our project directly addresses this urgent societal and economic challenge by proposing a transformative technology with the potential to generate significant, quantifiable impact across multiple domains.
...
\begin{itemize}
    ...
    \item \textbf{Enhanced Efficiency and Cost Savings:} The economic impact extends beyond reducing unnecessary treatments. Our framework's ability to automate the generation of structured radiological reports is projected to reduce the time radiologists spend on documentation by \textbf{24\%} per case \cite{Acosta_2024}. This translates to a significant productivity gain. Furthermore, by accelerating clinical research through in-silico trials, our platform can help reduce the enormous cost of pharmaceutical R\&D. A recent analysis suggests that AI-driven tools could reduce the cost of bringing a new oncology drug to market.
...
\begin{itemize}
    ...
    \item \textbf{Commercial Exploitation:} The primary route to market will be through a future spin-off company. This entity will provide enterprise-grade, validated versions of the software with long-term support, as well as consultancy services for adapting the framework to other diseases. This dual open-source/commercial model ensures both broad accessibility and a sustainable path to market.
\end{itemize}
...
\begin{itemize}
    \item \textbf{WP1: Data Curation and Harmonization (112 PMs):} This foundational work package is dedicated to the meticulous gathering, anonymizing, and harmonization of the multi-modal data that underpins the entire project. To ensure the dataset remains current and captures the latest clinical practices, we will hold yearly meetings with our urology, radiology, and nuclear medicine departments. The goal of these meetings is to verify whether standards of care have changed and, if so, to identify opportunities to incorporate new data to reflect these changes, such as collecting data from patients undergoing novel therapies.
    \textbf{Lead and Key Personnel:} Data Scientist, PhD Student 1, PhD Student 2.
    The tasks include:
    \begin{itemize}
        ...
        \item \textbf{Task 1.2: Data Structuring and Cleaning:} Develop and apply standardized protocols to clean and structure the diverse datasets. This includes normalizing clinical vocabulary, standardizing image acquisition parameters, and ensuring the integrity of all data fields. The protocols and tools for this task have been developed as part of the SIKIT project and will be reused here.
        \item \textbf{Task 1.3: Multi-Modal Data Linkage:} Create a unified patient record by accurately linking data from different sources (e.g., matching a patient's MRI scans with their corresponding EHR records and biopsy results) using a unique, anonymized identifier. The protocols and tools for this task have been developed as part of the SIKIT project and will be reused here.
        ...
        \item \textbf{Task 1.5: VAE-based Outlier Detection:} Develop and implement the VAE-based OOD detection mechanism to identify and flag potential outliers and label noise in the curated dataset. Establish a formal review loop with clinical experts to validate and correct the flagged data points.
    \end{itemize}
    \item \textbf{WP2: Foundational Supervisor Model Training (76 PMs):} Develop supervisor models for clinical ground truth. \textbf{Lead and Key Personnel:} Senior Researcher, Mathematician, PhD Student 2.
    \item \textbf{WP3: Causal VAE Development (60 PMs):} Develop the hierarchical \gls{vae} for disentanglement. \textbf{Lead and Key Personnel:} Senior Researcher, Mathematician, Programmer.
    \item \textbf{WP4: Temporal Trajectory Modeling (54 PMs):} Develop the \gls{njde} framework for longitudinal modeling. This work package will extend until the end of the project (M36). Towards the end of this period, the programmers will focus mainly on creating comprehensive documentation and refining the user interfaces for the clinical validation phase. \textbf{Lead and Key Personnel:} Senior Researcher, Mathematician, Programmer.
    \item \textbf{WP5: Validation and Clinical Integration (98 PMs):} Evaluate the framework's performance and clinical utility. This work package will now begin earlier, at month 15, to allow for a longer, more iterative validation process in parallel with the ongoing development in other work packages. \textbf{Lead and Key Personnel:} PI, Senior Researcher, PhD Student 3.
    \item \textbf{WP6: Dissemination, Communication, and Exploitation (16 PMs):} This work package will be managed in three 12-month cycles. It is dedicated to ensuring the project's results reach the widest possible audience and are positioned for maximum impact. Activities include publishing in high-impact journals, presenting at international conferences, maintaining the project website and open-source repositories, and engaging with clinical and patient communities. A detailed exploitation plan will be developed and refined throughout the project.
    \item \textbf{WP7: Project Management (44 PMs):} This work package will also be managed in three 12-month cycles to ensure continuous and effective project oversight. It covers all administrative, financial, and legal aspects of the project. A key responsibility is the implementation of the project's risk management plan and ensuring compliance with the EU AI Act through continuous monitoring and documentation.
    \item \textbf{WP8: Portfolio Activities (12 PMs):} This work package, structured in three 12-month cycles, is dedicated to fostering collaboration and synergy across the EIC Pathfinder Challenge portfolio. The project will actively participate in joint workshops, share best practices, and contribute to the development of a common strategic plan for the portfolio, ensuring our work aligns with and amplifies the impact of the broader EIC community.
\end{itemize}
...
\begin{table}[H]
    ...
    \begin{tabular}{lcccccccc|c}
        ...
        \midrule
        \textbf{Person-Months} & 112 & 76 & 60 & 54 & 98 & 16 & 44 & 12 & \textbf{468} \\
        \bottomrule
    \end{tabular}
\end{table}
...
\begin{table}[H]
    ...
    \begin{tabular}{lp{5.5cm}lcp{5cm}}
        ...
        M1.1 & Initial data collection and curation completed & WP1 & M12 & Curated dataset (D1.1) available with initial quality assurance report. \\
...
\begin{itemize}
    ...
    \begin{itemize}
        \item \textbf{Mitigation:} This is a core scientific challenge. Mitigation includes: 1) Iteratively refining the composite loss function with different weights for the penalization terms. 2) Exploring alternative disentanglement techniques (e.g., adversarial training). 3) Engaging external experts for a dedicated project review if initial methods fail.
    \end{itemize}
...
\begin{itemize}
    \item \textbf{The applicant institution:} As the coordinating institution, we provide the overall project lead, extensive expertise in AI model development for medical imaging, and contribute unique longitudinal and interim therapy imaging data. Our team will lead all technical WPs (WP1-WP5).
    ...
    \item \textbf{Two key partner radiological practices:} These partners provide access to a large cohort of primary diagnostic imaging (CT \& mpMRI), which is essential for training the supervisor models for early-stage disease in WP2.
\end{itemize}
The project will be led by Prof. Dr. Michael Kreißl, a renowned expert in nuclear medicine, who brings unparalleled leadership experience as the head of the working group for the S3-guidelines on thyroid cancer in Germany. This direct experience in shaping national clinical standards is a unique asset that will guide the project's clinical and regulatory strategy. The applicant's team has extensive, demonstrated experience in the core technologies required for this project, including synthetic CT creation from nuclear imaging, clinical decision support systems, and semi-automatic image segmentation pipelines. Current projects are showcased in our online portfolio. This deep clinical and technical expertise, combined with our strong collaborative network, uniquely positions us to succeed in this ambitious project.
...
\begin{itemize}
    ...
    \item \textbf{Student/Research Assistant (40hrs/month, 36 months):} \EUR{33,040}
\end{itemize}

\textbf{Total Estimated Personnel Costs: \EUR{2,855,626}}
...
\begin{itemize}
    ...
    \item \textbf{PhD Student (3 FTEs, 108 PMs):} The three PhD positions will be filled by medical students whose doctoral work is integral to the project's data integrity. Their clinical expertise is essential for bridging the gap between raw medical data and the technical AI team. Each student will have a distinct focus to ensure comprehensive data governance and validation throughout the project's lifecycle:
        \begin{itemize}
            \item \textbf{PhD Student 1 (Data Curation Lead):} This student will lead the data acquisition and standardization efforts within \textbf{WP1}. Their primary role involves managing the collection of multi-modal data from partner clinics, ensuring compliance with anonymization protocols, and standardizing diverse data formats. This requires a high level of medical understanding to correctly map clinical vocabularies and imaging parameters, a task far beyond the scope of a non-medical assistant.
            \item \textbf{PhD Student 2 (Quality Control & Ground Truth):} This student will focus on data quality control (\textbf{WP1}) and the establishment of clinical ground truth for the supervisor models (\textbf{WP2}). They will be responsible for implementing and clinically reviewing the outputs of the VAE-based outlier detection system (Task 1.5) and will perform the meticulous validation of imaging labels and clinical endpoints. This work demands doctoral-level clinical acumen to identify subtle data inconsistencies that could compromise model training.
            \item \textbf{PhD Student 3 (Clinical Validation Lead):} This student will lead the clinical validation studies in \textbf{WP5}. Their role involves working directly with clinicians to assess the plausibility of the model's outputs, including counterfactual images and prognostic reports. They will design and execute the workflow integration studies, requiring a broad understanding of both the AI technology and the realities of clinical practice to ensure the final tool is not only accurate but also useful.
        \end{itemize}
...
\subsubsection*{A.2 High-Performance Computing Costs (EuroHPC JU)}
The project will employ a hybrid computing model that combines secure, on-premise resources for data handling with a strategic utilization of Europe's sovereign supercomputing infrastructure for all high-performance computing (HPC) needs. This approach aligns the project with the EU's strategic goal of digital sovereignty while leveraging world-class, AI-optimized hardware. The budget of \textbf{\EUR{321,600}} is allocated to support the access and effective use of a leading EuroHPC Joint Undertaking (JU) supercomputer. This decision was reached after a careful comparison of various commercial and public compute providers, with a strong preference given to a Germany-based EuroHPC system to ensure close proximity to the project's primary data sources.

\paragraph{1. Strategic Imperative and Justification for EuroHPC JU Supercomputer Utilization}
A hybrid model utilizing the EuroHPC JU ecosystem is strategically superior for a flagship European research project. This approach provides a powerful and secure framework for medical AI innovation:
\begin{itemize}
    \item \textbf{On-Premise for Security (DIZ):} For critical reasons of data security and patient privacy, the main storage of raw clinical data and the initial preprocessing and anonymization steps will be handled locally within the secure environment of the applicant's Data Integration Center (DIZ). This leverages the validated tools from the SIKIT project, ensuring GDPR compliance.
    \item \textbf{EuroHPC for Sovereign, High-Performance AI:} Once data is fully anonymized, it will be transferred to a EuroHPC JU supercomputer for all large-scale computational tasks. This project's primary target is the \textbf{JUPITER} exascale system at the Jülich Supercomputing Centre, a flagship of European technological ambition. However, recognizing the dynamic nature of the HPC landscape, the project will apply for access to the most powerful and architecturally suitable EuroHPC system available at the project's start in early 2027, such as LUMI, LEONARDO, or MareNostrum 5. This strategy offers several key advantages:
    \begin{itemize}
        \item \textbf{Alignment with EU Strategy:} Utilising the EuroHPC infrastructure directly supports the EU's mission to achieve digital sovereignty and fosters a domestic technology ecosystem.
        \item \textbf{Access to World-Class, Specialized Hardware:} These systems provide access to state-of-the-art GPU architectures (e.g., NVIDIA GH200, AMD MI300A) that are purpose-built for extreme-scale AI and are often more powerful and cost-effective than commercial cloud offerings.
        \item \textbf{Integration with AI Factories:} By using a EuroHPC system, the project can leverage the comprehensive support services of the "AI Factories" initiative, including expert technical support and optimized software environments, accelerating the research and development lifecycle.
    \end{itemize}
\end{itemize}
This hybrid model ensures the highest level of data security through on-premise handling of sensitive information while harnessing the immense, AI-optimized power of Europe's sovereign supercomputing infrastructure for the core research tasks. The budget is allocated to cover all associated costs, including dedicated personnel time for code porting and optimization, data management and transfer, and any service fees required for large-scale, production-level access calls.

\textbf{Total Estimated High-Performance Computing Costs: \EUR{321,600}}
...
\begin{itemize}
    ...
    \item \textbf{Software Licenses:} A provision of \EUR{36,000} is budgeted for specialized software licenses. This covers subscriptions for AI-powered development tools (e.g., Gemini Ultra), literature analysis platforms (e.g., Scispace), and access to emerging proprietary models for comparative analysis and advanced LLMs to support various project tasks.
    ...
    \item \textbf{Other Work-Related Expenses:} A provision of \EUR{100,000} is budgeted for other essential work-related expenses, calculated at \EUR{10,000} per person for the 10 non-PhD members of the project team. This covers costs for specialized training, small equipment, and other unforeseen research-related needs that are critical for the project's success.
    \item \textbf{External Expert Consultations:} A provision of \EUR{15,000} is budgeted for consultations with external experts. This serves as a strategic contingency to address unforeseen scientific challenges, particularly in the high-risk development of the novel causal models (WP3, WP4).
\end{itemize}

\textbf{Total Estimated Other Direct Costs: \EUR{248,800}}
...
\textbf{Total Direct Costs (A): \EUR{3,458,526}}

\textbf{Indirect Costs (B = 25\% of A): \EUR{864,632}}

\subsubsection*{C. Total Project Budget}
The total requested funding is the sum of total direct and indirect costs.

\textbf{Total Estimated Project Cost (A + B):} \EUR{4,323,158}
...
\midrule
WP1: Data Curation & 675800 & 168950 & 844750 \\
WP2: Supervisor Models & 700000 & 175000 & 875000 \\
WP3: Causal VAE & 807500 & 201875 & 1009375 \\
WP4: Temporal Modeling & 807500 & 201875 & 1009375 \\
WP5: Validation & 200000 & 50000 & 250000 \\
WP6: Dissemination & 67726 & 16932 & 84658 \\
WP7: Project Management & 200000 & 50000 & 250000 \\
\midrule
\textbf{Total} & {\textbf{3458526}} & {\textbf{864632}} & {\textbf{4323158}} \\
\bottomrule
\end{tabular}
\end{table}
