...
\begin{itemize}
    ...
    \item \textbf{Risk 5: Model Drift and Performance Deterioration.} Performance of the AI algorithm could deteriorate over time ("model drift") due to changes in imaging equipment, protocols, or patient populations, a common challenge for clinical AI systems.
    \item \textbf{Mitigation:} Our core strategy to combat model drift is to ground the AI in fundamental biological and clinical knowledge, rather than allowing it to rely on spurious correlations. By explicitly modeling and disentangling scanner-type information and other technical confounders (Stage 2), and by training on diverse, multi-center data (WP1), we force the model to learn the underlying disease mechanisms, making it inherently more robust to superficial changes in the data landscape. Continuous monitoring (as described in our compliance with Article 15) will provide an early warning system to detect and address any performance degradation.
\end{itemize}
