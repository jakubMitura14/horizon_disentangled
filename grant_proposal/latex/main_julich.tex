...
Objective: To leverage advanced generative architectures—specifically Diffusion Models and Variational Autoencoders (VAEs)—to synthesize high-fidelity medical imagery (such as CT scans and dose maps). The project aims to enhance diagnostic precision while prioritizing patient safety.

Core Methodologies:
\begin{itemize}
    \item Generative Modeling: Utilizing Diffusion and VAE frameworks to create realistic synthetic datasets.
    \item Dose Map Synthesis: Generating accurate dose distributions to predict and optimize treatment planning.
    \item Generative Augmentation: Expanding limited medical datasets to improve the robustness of downstream AI tasks.
\end{itemize}

Key Goals:
\begin{itemize}
    \item Radiation Reduction: Minimizing patient exposure by synthesizing high-quality diagnostic images from low-dose inputs.
    \item Model Enhancement: Synthetic data will be used to train and refine Segmentation and Regression models.
\end{itemize}
...
\textbf{Instructions:} Not provided
...
\textbf{E-mail address:} michael.kreissl@med.ovgu.de

\textbf{Secondary e-mail address:} jakub.mitura@ovgu.de

\textbf{Nationality:} German

\textbf{Phone number:} Not provided

\textbf{Job title:} Professor / Head of Nuclear Medicine

\textbf{Employment contract valid for more than 3 months after end allocation:} Yes

\textbf{Website:} https://www.kmn.ovgu.de/
...
Prof. Dr. Michael Kreißl is a renowned expert in Nuclear Medicine with numerous publications in high-impact journals. The group has established expertise in AI applications for medical imaging.
...
Prior allocations on national clusters (NHR) for related projects.
...
Prof. Dr. Michael Kreißl (OVGU), Jakub Mitura, Joanna Wybrańska, Medical Faculty of OVGU Team.
...
\textbf{Requested amount of resources (node hours):} 25,000 Node Hours (approx. 100,000 GPU Hours)
...
\textbf{Maximum \# CPU cores per job:} Not provided

\textbf{Maximum number of jobs running simultaneously on the GPU partition:} 4

\textbf{Average \# CPU cores per job:} Not provided

\textbf{\# of CPU cores used per node:} Not provided
...
\textbf{Maximum amount of SCRATCH needed at a time (TB):} 200

\textbf{Maximum amount of WORK needed at a time (TB):} 50

\textbf{Maximum amount of HOME needed at a time (TB):} 1

\textbf{Maximum amount of ARCHIVE needed at a time (TB):} 200
...
\textbf{Total amount of data to transfer to/from (TB):} 200

\textbf{Justification of data transfer:} \\
Dataset: $\sim$200 TB (Raw/Processed). $\sim$100,000 DICOM/NIfTI files. Stored in HDF5 format for efficient parallel I/O. Access to JUPITER's Exascale capabilities is vital for training high-fidelity generative models on this large dataset.
...
\textbf{I/O data trafic R/W per hour:} 10 TB
...
We will port the Julia/Python hybrid workflow to JUPITER's architecture. Key focus: optimizing `solve` calls for NJDEs using `EnsembleGPUArray`, implementing distributed training for 3D Diffusion Models, and leveraging the Transformer Engine on H100 GPUs.
Hybrid implementation using both Julia (SciML) and Python (PyTorch/Monai).
...
Usage plan: 1. Profiling \& Porting (10\%): Adapting code to Grace-Hopper. 2. Scaling Tests (30\%): Weak/Strong scaling to 512+ nodes. 3. Production Training (60\%): Full-scale generation of synthetic cohorts.
...
All patient data is fully anonymized before entering the system, complying with GDPR and local regulations. Data usage is strictly for research purposes with appropriate ethics committee approval.
